# Next.js Frontend Phase 2 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build content management dashboard pages for carousel CRUD operations, slide editor, template management, scheduling interface, and analytics views.

**Architecture:** Client-side pages with server data fetching via SWR, Zustand for state management, drag-drop slide editor, calendar for scheduling, and chart components for analytics.

**Tech Stack:** Next.js 15, React 18, TypeScript, Tailwind CSS, SWR, Zustand, react-beautiful-dnd (drag-drop), react-calendar, Recharts (charts)

**Spec:** `SECTION_5_FINAL_IMPLEMENTATION_PLAN.md` Phases 2-5, backend API endpoints verified

---

## Global Constraints

- All pages must be inside (dashboard) route group (protected by middleware)
- TypeScript strict mode enabled
- Use SWR for data fetching with proper error handling
- API calls use apiClient from Phase 1
- Components should be client-side ('use client') for interactivity
- Tailwind CSS only, no additional CSS libraries
...
1. **SWR stale data during rapid updates**: Multiple carousel updates in quick succession must sync properly
2. **Drag-drop slide reordering fails**: Reordering slides must persist to backend without duplication
3. **Calendar timezone handling**: Scheduled times must respect user timezone
4. **Missing error states**: API failures on content operations must show graceful error UI
5. **Performance on large carousel lists**: Pagination and virtualization needed for 100+ carousels

---

## Task Breakdown

### Task 1: Create Content Store (Zustand) with CRUD actions
### Task 2: Create Carousel List Page with pagination
### Task 3: Create Carousel Editor Page (create/edit)
### Task 4: Create Slide Editor with drag-drop
### Task 5: Create Template Management page
### Task 6: Create Schedule/Calendar page
### Task 7: Create Analytics Dashboard page
### Task 8: Add real-time Socket.io updates
### Task 9: Complete page routing and navigation
### Task 10: Integration testing and refinement

---

## Task 1: Create Content Store (Zustand)

**Files:**
- Create: `src/store/contentStore.ts`
- Create: `src/store/__tests__/contentStore.test.ts`

**Interfaces:**
- Produces: `useContentStore` with carousel/slide/template CRUD
- Consumes: `apiClient` from Phase 1

- [ ] **Step 1: Create content store**
...
interface ContentState {
  ...
  reorderSlides: (carouselId: string, slides: Slide[]) => Promise<void>;
...
export const useContentStore = create<ContentState>((set, get) => ({
  ...
  reorderSlides: async (carouselId, slides) => {
    set({ isLoading: true, error: null });
    try {
      set({ slides });
      // Backend will handle reordering via slide_number updates
    } catch (error: any) {
...
- [ ] **Step 2: Commit content store**

```bash
git add src/store/contentStore.ts
git commit -m "feat: Add content store with carousel, slide, template CRUD operations"
```

---

## Task 2: Create Carousel List Page

**Files:**
- Create: `src/app/(dashboard)/content/page.tsx`
- Create: `src/components/content/CarouselCard.tsx`
- Create: `src/components/content/CarouselList.tsx`
